...
export default function Home() {
  ...
  const canManageOmnimesh = isAuthenticated && user?.role === "admin";
  useEffect(() => {
    if (!localStorage.getItem("omnimesh_autonomous_guide_seen")) setShowGuide(true);
  }, []);

  const metricsQuery = trpc.omnimesh.metrics.useQuery(undefined, { enabled: canManageOmnimesh, refetchInterval: 5000 });
  const eventsQuery = trpc.omnimesh.listEvents.useQuery(undefined, { enabled: canManageOmnimesh, refetchInterval: 5000 });
  const destinationsQuery = trpc.omnimesh.listDestinations.useQuery(undefined, { enabled: canManageOmnimesh, refetchInterval: 10000 });
  const securityQuery = trpc.omnimesh.securityStatus.useQuery(undefined, { enabled: canManageOmnimesh, refetchInterval: 10000 });
  const configQuery = trpc.omnimesh.getConfig.useQuery(undefined, { enabled: canManageOmnimesh, refetchInterval: 10000 });
  ...
  if (authLoading) {
    return <div className="min-h-screen bg-slate-950 text-slate-300 flex items-center justify-center">Loading secure workspace…</div>;
  }

  if (!isAuthenticated) {
    return <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6"><Card className="w-full max-w-md border-slate-800 bg-slate-900"><CardHeader><CardTitle>Sign in to OmniMesh</CardTitle><CardDescription>Webhook delivery history and recovery controls are restricted to your workspace administrator.</CardDescription></CardHeader><CardContent><Button onClick={startLogin} className="w-full bg-emerald-500 text-slate-950 hover:bg-emerald-400">Sign in securely</Button></CardContent></Card></div>;
  }

  if (!canManageOmnimesh) {
    return <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6"><Card className="w-full max-w-md border-slate-800 bg-slate-900"><CardHeader><CardTitle>Administrator access required</CardTitle><CardDescription>Your account is signed in but is not authorized to view webhook payload history or operate recovery controls.</CardDescription></CardHeader></Card></div>;
  }
  ...
  return (
    <div className="omni-shell min-h-screen text-slate-100 bg-slate-950 font-sans">
      ...
      {/* Webhook security modal */}
      <Dialog open={showSettings} onOpenChange={setShowSettings}>
        <DialogContent className="max-w-md border-slate-700 bg-slate-950 text-white">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl font-bold"><Lock className="h-5 w-5 text-emerald-400" /> Webhook Verification</DialogTitle>
            <DialogDescription className="text-xs text-slate-400">Signing secrets are loaded from the server environment and are never saved in the OmniMesh database.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-3 text-sm">
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
              <p className="font-semibold text-white">Stripe</p>
              <code className="mt-2 block rounded bg-slate-950 px-2 py-1.5 text-xs text-cyan-300">STRIPE_WEBHOOK_SECRET=whsec_...</code>
              <p className="mt-2 text-xs leading-5 text-slate-400">For local testing, run Stripe CLI with <code>stripe listen --forward-to http://localhost:3000/webhooks/stripe</code> and use the secret it prints.</p>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
              <p className="font-semibold text-white">Shopify</p>
              <code className="mt-2 block rounded bg-slate-950 px-2 py-1.5 text-xs text-cyan-300">SHOPIFY_API_SECRET=your_app_client_secret</code>
              <p className="mt-2 text-xs leading-5 text-slate-400">Use the app client secret that Shopify uses to generate the <code>X-Shopify-Hmac-SHA256</code> header.</p>
            </div>
          </div>
          <DialogFooter className="pt-3">
            <Button type="button" onClick={() => setShowSettings(false)} className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold">Done</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <header className="relative z-10 mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <div className="flex items-center gap-3">
          <Button size="sm" variant="outline" onClick={() => setShowSettings(true)} className="border-slate-800 bg-slate-900/60 text-slate-300 hover:bg-slate-800">
            <Lock className="h-3.5 w-3.5 mr-1.5 text-emerald-400" /> {signatureVerified ? "Verification status" : "Webhook setup"}
          </Button>
          ...
          <div className={`flex items-center gap-2 text-xs px-3 py-1.5 rounded-full border ${signatureVerified ? "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" : "text-amber-300 bg-amber-500/10 border-amber-500/20"}`}><span className={`h-2 w-2 rounded-full ${signatureVerified ? "bg-emerald-400 animate-pulse" : "bg-amber-300"}`} /> {signatureVerified ? "Verification active" : "Verification required"}</div>
        </div>
      </header>
